defmodule Tradenara.AuthTest do
  use Tradenara.ConnCase
  alias Tradenara.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Tradenara.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Tradenara.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Tradenara.User{id: 123})
      |> send_resp(:ok, "")
